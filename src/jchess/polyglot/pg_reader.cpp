...
namespace jchess::polyglot {
    ...
    int PGZobristHasher::get_castle_offset(CastleBits castle_type) const {
        if(castle_type == 0) {
            return NO_OFFSET;
        }
        switch(castle_type) {
    ...
    int PGZobristHasher::get_enp_offset(const Board &board) const {
        // weird polyglot en passant quirk - need to actually be able to capture into the enp square.
        if(!board.can_enp_capture()) {
            return NO_OFFSET;
        }
        Square enp = board.get_board_state().enp_square.value();
        return file_of(enp);
    }

    int PGZobristHasher::get_piece_offset(Square square, Piece piece) const {
        if(piece == NO_PIECE) {
            return NO_OFFSET;
        }
        int piece_kind = pg_piece_kinds[piece];
        auto [rank, file] = rank_file_from_square(square);
        return 64*piece_kind + 8*rank + file;
    }

    PGZobristHasher::PGZobristHasher()
        : ZobristHasher(pg_hash_values, PG_PIECE_START, PG_CASTLE_START, PG_ENP_START, PG_TURN_START)
        {}

    Move read_pg_move(uint16_t move, Board const& board) {
        int dest_file = move & 0b111;
        int dest_rank = (move >> 3) & 0b111;
        int src_file = (move >> 6) & 0b111;
        ...
        if(board.get_board_state().pieces[src] == (KING | board.get_side_to_move())) {
            if (src == E1) {
                ...
                dest = (dest == A1) ? C1 : dest;
            }
            if (src == E8) {
                dest = (dest == H8) ? G8 : dest;
                dest = (dest == A8) ? C8 : dest;
            }
        }

        return {src, dest, promotion};
    }

    PGMappedBook::PGMappedBook(std::string const& pg_book_path) {
        std::error_code err_code;
        mapping.map(pg_book_path, 0, mio::map_entire_file, err_code);
        if(err_code) {
            throw std::runtime_error("could not open opening book");
        }
    }

    void PGMappedBook::map_file(std::string const& pg_book_path) {
        std::error_code err_code;
        mapping.map(pg_book_path, 0, mio::map_entire_file, err_code);
        if(err_code) {
            throw std::runtime_error("could not open opening book");
    ...
    size_t PGMappedBook::size() const {
        return mapping.size() / BOOK_ENTRY_BYTES;
    }

    PGBookEntry PGMappedBook::operator[](size_t index) const {
        // surely better way?
        const uint8_t *book_start = mapping.data();
        const uint8_t *key_start = book_start + index * BOOK_ENTRY_BYTES;
        const uint8_t *move_start = key_start + 8;
        const uint8_t *weight_start = move_start + 2;
        const uint8_t *learn_start = weight_start + 2;

        auto key = read_big_endian<uint64_t>(key_start);
        auto move = read_big_endian<uint16_t>(move_start);
        auto weight = read_big_endian<uint16_t>(weight_start);
        auto learn = read_big_endian<uint32_t>(learn_start);

        return {key, move, weight, learn};
    }

    PGBookIterator PGMappedBook::begin() const {
        return {this, 0};
    }

    PGBookIterator PGMappedBook::end() const {
        return {this, size()};
    }

    bool operator==(PGBookIterator const& lhs, PGBookIterator const& rhs) {
        return lhs.cur_index == rhs.cur_index;
    }

    std::optional<Move> PGMappedBook::get_random_book_move(Board const& board) const {
        uint64_t key = hasher.hash_board(board);
        auto lower = std::lower_bound(begin(), end(), PGBookEntry{key, 0, 0, 0});
        if(lower == end() || (*lower).key != key) {
            return std::nullopt; // no entries with this hash
        }
        auto upper = std::upper_bound(begin(), end(), PGBookEntry{key, 0, 0, 0});
    ...
    PGBookIterator& PGBookIterator::operator++() {
        ++cur_index;
        return *this;
    }

    PGBookEntry PGBookIterator::operator*() {
        return (*book)[cur_index];
    }

    PGBookIterator& PGBookIterator::operator=(PGBookIterator const& other) {
        if(this == &other) {
            return *this;
        }
