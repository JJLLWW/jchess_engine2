...
#define PCOLOR(p) ((p)&1)
    ...
    int convert_piece_to_pesto(jchess::Piece piece) {
        switch(piece) {
            case jchess::W_PAWN:
                return WHITE_PAWN;
            case jchess::B_PAWN:
                return BLACK_PAWN;
            case jchess::W_KNIGHT:
                return WHITE_KNIGHT;
            case jchess::B_KNIGHT:
                return BLACK_KNIGHT;
            case jchess::W_BISHOP:
                return WHITE_BISHOP;
            case jchess::B_BISHOP:
                return BLACK_BISHOP;
            case jchess::W_ROOK:
                return WHITE_ROOK;
            case jchess::B_ROOK:
...
/* values from Rofchade: http://www.talkchess.com/forum3/viewtopic.php?f=2&t=68311&start=19 */
    ...
    int eval(std::array<jchess::Piece, 64> const& board_jc, int side2move)
    {
        ...
        for(int i=0; i<64; ++i) {
            board[i] = convert_piece_to_pesto(board_jc[i]);
        }
        int mg[2];
        int eg[2];
        int gamePhase = 0;
        ...
        // what is the minimum and maximum score here?
